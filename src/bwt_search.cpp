...
bwtSearchResult_t BWT_Search(uint8_t* seq, int start, int stop)
{
	...
	bwtSearchResult.freq = bwtSearchResult.len = 0; bwtSearchResult.LocArr = NULL;
...
//#define KmerSize 8
//
//vector<FragPair_t> SimplePairRescue(int64_t LowerBound, int64_t UpperBound, ReadItem_t& read)
//{
//	int64_t gPos;
//	FragPair_t FragPair;
//	bwtintv_t ik, ok[4];
...
//	int i, p, pos, stop_pos, n_occur;
//
//	FragPair.bSimple = true;
//	for (pos = 0, stop_pos = KmerSize; pos < read.rlen; stop_pos += KmerSize)
//	{
//		p = (int)read.EncodeSeq[pos];
//		ik.x[0] = Refbwt->L2[p] + 1;
//		ik.x[1] = Refbwt->L2[3 - p] + 1;
//		ik.x[2] = Refbwt->L2[p + 1] - Refbwt->L2[p];
//
//		for (; pos < stop_pos; pos++)
//		{
//			if (read.EncodeSeq[pos] > 3) break;// ambiguous base
//			bwt_2occ4(Refbwt, ik.x[1] - 1, ik.x[1] - 1 + ik.x[2], tk, tl);
//			for (i = 0; i != 4; ++i) {
//				ok[i].x[1] = Refbwt->L2[i] + 1 + tk[i];
//				ok[i].x[2] = tl[i] - tk[i];
...
//			ok[0].x[0] = ok[1].x[0] + ok[1].x[2];
//
//			i = 3 - read.EncodeSeq[pos];
//			if (ok[i].x[2] == 0) break; // extension ends
//			else ik = ok[i];
//		}
//		if (pos == stop_pos)
//		{
//			if ((n_occur = (int)ik.x[2]) > 1000) n_occur = 0;
//			for (i = 0; i < n_occur; i++)
//			{
//				gPos = bwt_sa(ik.x[0] + i);
//				printf("Found %lld\n", gPos);
//				if (gPos > LowerBound && gPos < UpperBound)
//				{
//					FragPair.rPos = stop_pos - KmerSize;
//					FragPair.gPos = gPos;
//					FragPair.PosDiff = FragPair.gPos - FragPair.rPos;
//					SPvec.push_back(FragPair);
//				}
//			}
//		}
//		else pos = stop_pos;
